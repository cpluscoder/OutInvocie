...
应用程序向导已为您创建了此 Discover 项目，作为编写服务 (EXE) 的起点。
...
Discover.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(Discover.h)
        GUID 定义			(Discover_i.c)
        类型库				(Discover.tlb)
        封送处理代码			(Discover_p.c 和 
                                           dlldata.c）

Discover.h
    此文件包含 Discover.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 Discover.pch 的预编译头 (PCH) 文件和
...
Discoverps.vcxproj
    ...
    文件。此进程生成 dlldata.c、Discover_i.c 和 
    Discover_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

Discoverps.vcxproj.filters
