...
应用程序向导已为您创建了此 Upgrade 项目，作为编写服务 (EXE) 的起点。
...
Upgrade.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(Upgrade.h)
        GUID 定义			(Upgrade_i.c)
        类型库				(Upgrade.tlb)
        封送处理代码			(Upgrade_p.c 和 
                                           dlldata.c）

Upgrade.h
    此文件包含 Upgrade.idl 中定义的项目的 C++ 接口定义
...
Upgradeps.vcxproj
    ...
    文件。此进程生成 dlldata.c、Upgrade_i.c 和 
    Upgrade_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

Upgradeps.vcxproj.filters
